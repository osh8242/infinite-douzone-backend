package com.douzone.rest.empadd.vo;

import lombok.Data;

@Data
public class EmpAdd {
    private String cdEmp;
    private String nmChName;
    private String nmEnName;
    private String ynDrawContracts;
    private String dcMidRetire;
    private String fgWedding;
    private String fgDisorder;
    private String daBirth;
    private String strHobby;
    private String nmSpecial;
    private String tyReligion;
    private String zipHome;
    private String zipOrgin;
    private String addOrgin1;
    private String addOrgin2;
    private String mnKeepGarden;
    private String mnKeepEstate;
    private String fgOwnHouse;
    private String telOffice1;
    private String telOffice2;
    private String telOffice3;
    private String telExtens;
    private String tyMilit;
    private String whyMltOff;
    private String daMltBegin;
    private String daMltEnd;
    private String fgMltType;
    private String nmMltBranc;
    private String strMltDisch;
    private String cdMltRank;
}

